...
function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // Show whatever error the backend returned (e.g. email already registered)
  ...
  return (
    <div className="min-h-screen hero-gradient transition-colors flex items-center justify-center p-4 relative overflow-hidden">
      <div className="absolute -top-24 -right-24 w-72 h-72 rounded-full bg-amber-400/15 blur-3xl" />
      ...
      >
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{t('labelPassword')}</label>
            <input 
              type="password" 
        ...
        <p className="text-center text-sm text-gray-600 dark:text-gray-400 mt-8">
          {t('haveAccount')}{' '}
          <Link to="/login" className="font-semibold text-blue-600 dark:text-amber-400 hover:underline">
            {t('loginHere')}
          </Link>
        </p>
